...
export interface ImporterOfRecord
	extends Pick<
			Address,
			"address_line1" | "city_locality" | "postal_code" | "country_code"
		>,
		Partial<
			Pick<
				Address,
				"phone" | "email" | "address_line2" | "address_line3" | "state_province"
			>
		> {
	/**
...
type AddressValidatingShipment = Shipment &
	Pick<PurchaseLabelOptions, "validate_address">;
...
interface GetRatesByShipmentOptions extends GetRatesBaseRequest {
	 ...
	 */
	shipment: Partial<AddressValidatingShipment> &
		Required<Pick<AddressValidatingShipment, "ship_to">>;
	shipment_id?: never;
}

export type GetRatesOptions =
	| GetRatesByShipmentIdOptions
	| GetRatesByShipmentOptions;
...
export type EstimateRatesOptions =
	| EstimateRatesByCarrierIdRequest
	| EstimateRatesByCarrierIdsRequest;

export type EstimateRatesResponse = Array<
	Omit<Rate, "rate_id" | "requested_comparison_amount">
>;
